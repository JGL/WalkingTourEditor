...
function addAddWaypointButton() {
  ...
  if (checkForGeolocation()) {
    ...
    waypointMultimedia.value =
      "https://reasons.to/content/0-2009/1-brighton/1-speakers/0-joel-gethin-lewis/joel-gethin-lewis.jpg";
    ...
    waypointLabel.addEventListener("keyup", event => {
      const isValidLabel =
        ...
        event.srcElement.value != promptText; //only checking if any text is added at the moment
...
async function getClientSideTour() {
  ...
  console.log(
    `serverTour at the start of getClientSideTour is ${JSON.stringify(
      serverTour
    )}`
  );
  ...
  for (let waypointDivision of waypointsList.children) {
    ...
    currentWaypoint.label = waypointDivision.querySelector(
      "div.waypoint-label"
    ).innerHTML; //https://developer.mozilla.org/en-US/docs/Web/API/Element/querySelector / https://developer.mozilla.org/en-US/docs/Web/API/Document/querySelector
    //important that these are numbers, not text
    currentWaypoint.latitude = Number(
      waypointDivision.querySelector("div.waypoint-latitude-hidden").innerHTML
    );
    currentWaypoint.longitude = Number(
      waypointDivision.querySelector("div.waypoint-longitude-hidden").innerHTML
    );
    currentWaypoint.timestamp = Number(
      waypointDivision.querySelector("div.waypoint-timestamp-hidden").innerHTML
    );
    currentWaypoint.multimedia = waypointDivision.querySelector(
      "div.waypoint-multimedia"
    ).innerHTML;
    currentWaypoint.range = Number(
      waypointDivision.querySelector("div.waypoint-range").innerHTML
    );
...
async function displayTour() {
  ...
  while (waypoints.firstChild) {
    // make sure none of the  old information is there.... https://stackoverflow.com/questions/3955229/remove-all-child-elements-of-a-dom-node-in-javascript
  ...
  for (let waypoint of tour.waypoints) {
    ...
    rubbishIcon.addEventListener("click", async event => {
      ...
      if (userConfirmed) {
        ...
        console.log(
          `rubbishIcon: The edited tour is ${JSON.stringify(editedTour)}`
        );
        saveTour(editedTour);
